...
interface Invoice {
  ...
  dueDate: string;
  memo: string;
  status: "draft" | "pending" | "paid" | "review";
  paymentUrl: string;
}
...
const topbarBreadcrumb = document.getElementById("topbarBreadcrumb") as HTMLElement;
...
const breadcrumbIcons: Record<string, string> = {
...
function renderInvoicesTable(filteredInvoices: Invoice[]): void {
  const overviewBody = getElement<HTMLTableSectionElement>("overviewInvoiceTableBody");
  const invoiceViewBody = getElement<HTMLTableSectionElement>("invoicesTableBody");
  ...
  getElement<HTMLElement>("sidebarInvoiceCount").textContent = invoices.length.toString();
  getElement<HTMLElement>("sidebarSettlementCount").textContent = paidCount.toString();
}
...
function updateMetrics(): void {
  ...
  getElement<HTMLElement>("cumulativeSettledVal").textContent = formatUsd(totalSettled);
  getElement<HTMLElement>("pendingInvoicesVal").textContent = formatUsd(totalPending);
  ...
  getElement<HTMLElement>("invoicesDraftSum").textContent = formatUsd(mockupDraftSum);
  getElement<HTMLElement>("invoicesDraftCount").textContent = `${mockupDraftCount} invoices`;

  getElement<HTMLElement>("invoicesDueSum").textContent = formatUsd(mockupDueSum + addedDueAmount);
  getElement<HTMLElement>("invoicesDueCount").textContent = `${mockupDueCount + addedDueCount} invoices`;
}

// Sync state on load from API
async function loadInvoicesFromApi(): Promise<void> {
  try {
    ...
    if (res.ok) {
      ...
      if (data && Array.isArray(data.invoices) && data.invoices.length > 0) {
        ...
        const apiInvoices: Invoice[] = data.invoices.map(i => ({
          id: i.id,
          client: i.client,
          amount: i.amount,
          asset: i.asset || "USDC",
...
// Modal handling
const invoiceModal = getElement<HTMLDivElement>("invoiceModal");
...
const clientInput = getElement<HTMLInputElement>("clientName");
const amountInput = getElement<HTMLInputElement>("invoiceAmount");
const dueDateInput = getElement<HTMLInputElement>("dueDate");

function updateCheckoutPreview(): void {
  ...
  getElement<HTMLElement>("previewClient").textContent = clientName;
  getElement<HTMLElement>("previewAmount").textContent = formatUsd(amount);
  getElement<HTMLElement>("previewDueDate").textContent = dateVal;
}
...
// Modal triggers
getElement<HTMLElement>("globalNewInvoiceBtn").addEventListener("click", openModal);
getElement<HTMLElement>("invoicesViewCreateBtn").addEventListener("click", openModal);
getElement<HTMLElement>("heroCreate").addEventListener("click", openModal);
getElement<HTMLElement>("invoiceModalClose").addEventListener("click", closeModal);
...
async function createInvoiceSubmit(): Promise<void> {
...
getElement<HTMLButtonElement>("createInvoice").addEventListener("click", createInvoiceSubmit);

// Copy actions
function handleCopyText(text: string, buttonElement: HTMLElement, successText = "Copied!"): void {
...
// Copy Code Snippet
const apiCopySnippetBtn = getElement<HTMLButtonElement>("apiCopySnippetBtn");
apiCopySnippetBtn.addEventListener("click", () => {
  const snippet = getElement<HTMLElement>("apiCodeSnippet").textContent || "";
...
document.querySelectorAll<HTMLButtonElement>("[data-copy-target]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (targetId) {
      const addressText = getElement<HTMLElement>(targetId).textContent || "";
...
const globalSearch = getElement<HTMLInputElement>("globalSearch");
const invoiceSearchInput = getElement<HTMLInputElement>("invoiceSearchInput");
...
function handleSearch(query: string): void {
  ...
  const filtered = invoices.filter(
    (inv) =>
      inv.client.toLowerCase().includes(trimmed) ||
